                ...
                if (state[article.key].value.length === 1) {
                  ...
                  state[article.key].price = state[article.key].value.reduce((a, b) => {
                    if (a.price) {
                      ...
                      if (b.language === data.defaultLanguage) {
                        return a.price;
                      }
    ...
    return (
      <div className={styles.languageSelectorContainer} key={`article-${i}`}>
        ...
        {this.state[article.key].value.map((e, i) => {
          const languageData = this.props.data.languages.filter((language, i) => language.key === e.language);
          return (
            <div key={i}>
              <img
              ...
              {e.language !== this.props.data.defaultLanguage
                            ...
                            <span
                              ...
                              onClick={() => {
                                const newState = this.state[article.key].value.filter(lang => (lang.language !== e.language));
                                const newPrice = newState.length === 1 ? 0 : newState.reduce((a, b) => {
                                  if (a.price) {
                                    if (a.language === this.props.data.defaultLanguage) {
                                      return b.price;
                                    } if (b.language === this.props.data.defaultLanguage) {
                                      return a.price;
                                    }
                                    return a.price + b.price;
                                  } if (a.price === undefined && b === undefined) {
                                    return a;
                                  }
                                  return a + b.price;
                                });
        ...
        {this.props.data.languages.length > this.state[article.key].value.length
                && (
                <div
                  className={cx(styles.addLanguageButton, !this.state.moreLanguagesBookable && styles.invisible)}
                  onClick={(e) => {
                    this.setState({
        ...
        {this.state.showLanguageSelector
                ...
                <div className={cx(styles.languageList)}>
                  {this.props.data.languages.map((e, i) => {
                    const exclude = this.state[article.key].value.filter(obj => obj.language === e.key);
                    if (exclude.length > 0) {
                      return null;
                    }
...
€
  ...
  renderArticle(article, i) {
    this.purchaseData.articles = this.purchaseData.articles.filter(purchase => (purchase.key !== article.key));
    ...
    switch (article.key) {
      ...
      case 'participants':
        ...
        return (
          <div className={styles.dropdown} key={`article-${i}`}>
            <SelectMenu
              defaultValue={this.state[article.key].value}
              options={options}
              onChange={(obj) => {
                this.setState({
      ...
      case 'analytics_monitoring':
        ...
        if (checked) {
          this.purchaseData.articles = this.purchaseData.articles.filter(purchase => (purchase.text !== this.t(`priceTable.articles.${article.key}`)));
      ...
      case 'custom_domain':
        return (
          ...
          >
            <Icon
              fixedWidth
              name={this.state[article.key].value ? 'check' : 'close'}
  ...
  renderCustomCols(column, i) {
    return (
      <Col md="12" lg="4" key={`col-${i}`} className={styles.column}>
        ...
        {column.map(this.renderArticle.bind(this))}
      </Col>
    );
  }
        ...
        <Col xs="12">
          <div className={styles.customPackageInnerContainer}>
            ...
            <Row className={styles.customRow}>
              {data.plans.custom.articles.map(this.renderCustomCols.bind(this))}
            </Row>
            <Row>
              {this.renderButton()}
            </Row>
